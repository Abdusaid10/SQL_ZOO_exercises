...
--  the places you can get to from Craiglockhart, without changing
--  routes. Change the query so that it shows the services from
--  Craiglockhart to London Road.
SELECT a.company, a.num, a.stop, b.stop
FROM route a JOIN route b ON
  (a.company=b.company AND a.num=b.num)
WHERE a.stop=53 AND b.stop=149
-- 6 oining two copies of the stops table we can refer to stops
-- by name rather than by number. Change the query so that the 
...
FROM route a
    JOIN route b ON (a.num=b.num) JOIN stops stopa ON(a.stop=stopa.id)
    JOIN stops stopb ON(b.stop=stopb.id)
